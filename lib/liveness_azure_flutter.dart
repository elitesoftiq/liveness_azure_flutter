...
class LivenessAzureFlutter {
  ...
  static Future<LivenessSession?> createSession(
      ...
      String apiVersion = 'v1.2-preview.1'}) async {
    final verifyImage = imagePath != null;
    final deviceUID = deviceCorrelationId ??
        await LivenessAzureFlutterPlatform.instance.getDeviceUID();
    if (deviceUID == null) {
      throw Exception('deviceCorrelationId cannot be null');
    }

    final detectType =
        verifyImage ? 'detectLivenessWithVerify' : 'detectLiveness';
    final createSessionUri =
        '$faceApiEndpoint/face/$apiVersion/$detectType/singleModal/sessions';
    ...
    if (response.statusCode != null &&
      ...
      if (data['authToken'] != null) {
        return LivenessSession(
            authSession: data['authToken'], sessionId: data['sessionId']);
      }
    }
  ...
  static Future<LivenessResult?> initLiveness(
      ...
      LivenessTheme theme = const LivenessTheme()}) async {
    final result = await LivenessAzureFlutterPlatform.instance
        .initLiveness(authTokenSession, theme: theme);
